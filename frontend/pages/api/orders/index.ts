  ...
  if (orderData) {
    ...
    try {
      ...
      try {
        ...
        console.log('API Proxy: Статус ответа от базы данных:', dbResponse.status);
        console.log('API Proxy: Тип данных ответа:', typeof dbResponse.data);
        
        if (dbResponse.status === 200 && dbResponse.data) {
          console.log('API Proxy: Получены данные заказов из базы данных');
          
          // Получаем ID заказов для запроса товаров
          const orders = dbResponse.data || [];
          if (orders.length > 0) {
            console.log(`API Proxy: Получено ${orders.length} заказов, получаем товары для них`);
            ...
            try {
              ...
              console.log('API Proxy: Статус ответа запроса товаров:', itemsResponse.status);
              
              if (itemsResponse.status === 200 && itemsResponse.data) {
                console.log(`API Proxy: Получены товары заказов, количество: ${itemsResponse.data.length}`);
            ...
            console.log('API Proxy: Заказы не найдены, возвращаем пустой массив');
            return res.status(200).json([]);
          }
        } else {
          console.log('API Proxy: Не удалось получить данные из базы данных, возвращаем пустой массив');
          console.log('API Proxy: Данные ответа:', JSON.stringify(dbResponse.data).substring(0, 300));
          
          // Возвращаем пустой массив вместо ошибки
          return res.status(200).json([]);
        }
      } catch (dbRequestError: any) {
        console.log('API Proxy: Ошибка при запросе к базе данных:', dbRequestError.message);
        // Возвращаем пустой массив вместо ошибки
        return res.status(200).json([]);
      }
    } catch (dbError: any) {
