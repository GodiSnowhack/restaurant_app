...
/**
 * API-прокси для создания отзыва о заказе
 */
export default async function createOrderReviewProxy(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const endpoint = `${apiUrl}/reviews/order`;
    
    console.log(`Reviews API - Отправка запроса на создание отзыва о заказе`, req.body);
    ...
    try {
      ...
      console.log(`Reviews API - Успешное создание отзыва о заказе`);
    ...
    console.error('Reviews API - Ошибка при создании отзыва о заказе:', error.message);
