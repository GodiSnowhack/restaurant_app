...
/**
 * API-маршрут для обновления статуса оплаты заказа официантом
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Получаем статус оплаты из тела запроса (или устанавливаем 'paid' по умолчанию)
    const { status = 'paid' } = req.body;

    console.log(`Waiter API: Подтверждение оплаты заказа #${id} (статус: ${status})`);
    ...
    // Формируем URL для запроса - используем правильный эндпоинт
    const url = `${baseApiUrl}/waiter/orders/${id}/confirm-payment`;
    ...
    try {
      const response = await axios.post(
        url,
        {},  // Пустое тело, так как эндпоинт не требует дополнительных параметров
        { 
          headers: {
      ...
      return res.status(200).json({
        success: true,
        message: 'Оплата заказа подтверждена',
        order: {
