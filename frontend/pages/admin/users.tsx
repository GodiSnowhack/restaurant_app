      ...
      <div className="container mx-auto px-4 py-8 dark:bg-gray-900 min-h-screen">
        ...
        {filteredUsers.length === 0 ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
            <h2 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white">Список пользователей</h2>
            <div className="overflow-x-auto">
              <table className={`min-w-full divide-y ${isDark ? 'bg-gray-800 divide-gray-700' : 'bg-white divide-gray-200'}`}>
                <thead className={isDark ? 'bg-gray-900/50' : 'bg-gray-50'}>
                  <tr>
                    <th scope="col" className={`px-2 py-2 text-left text-xs font-medium ${isDark ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>
                      ID
                    </th>
                    <th scope="col" className={`px-2 py-2 text-left text-xs font-medium ${isDark ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wider`}>
                      Пользователь
                  ...
                  {filteredUsers.map((user) => (
                    ...
                    >
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>{user.id}</div>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className={`flex items-center`}>
                          <div className={`flex-shrink-0 h-7 w-7 bg-gray-200 ${isDark ? 'bg-gray-700' : ''} rounded-full flex items-center justify-center`}>
                            <UserIcon className="h-4 w-4 text-gray-400" />
                          </div>
                          <div className="ml-2">
                            <div className={`text-xs font-medium ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{user.full_name}</div>
                            <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{user.email}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className={`text-xs ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{user.phone || '-'}</div>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className="flex items-center">
                          <div className="relative group">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-${roleColors[user.role] || 'gray'}-${isDark ? '900/50' : '100'} text-${roleColors[user.role] || 'gray'}-${isDark ? '200' : '800'} cursor-pointer`}>
                              {roleIcons[user.role] || <UserIcon className="h-3 w-3 mr-1" />} {roleDisplayNames[user.role] || user.role}
                            </span>
                          </div>
                        </div>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className={`text-xs ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{translateAgeGroup(user.age_group)}</div>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.is_active ? isDark ? 'bg-green-900/50 text-green-200' : 'bg-green-100 text-green-800' : isDark ? 'bg-red-900/50 text-red-200' : 'bg-red-100 text-red-800'}`}>{user.is_active ? 'Активен' : 'Неактивен'}</span>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className={`text-xs ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{formatDate(user.created_at)}</div>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap text-xs">
                        <div className={`text-xs ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{formatDate(user.updated_at)}</div>
                      </td>
